...
public class JangGoonCheck : MonoBehaviour {
    ...
    public void jangGoonCheck(string sensor_name, string mal_name) {
    ...
    private void malSearch(int row, int col) {
    ...
    bool searchPath(int row, int col, int dir) {
        ...
        if (row <= 0 || row >= 10 || col <= 0 || col >= 11) { // 범위 밖
            ...
            if (tmp_obj.GetComponent<SensorControl>().getExistsFlag()) { // 센서 위에 뭐가 있으면
                ...
                if (dir == GO) { // 경로
                    if (tmp_name == "Han_Po(Clone)" || tmp_name == "Cho_Po(Clone)")
                        return true;
                    else
                        return false;
                }
                else { // 도착지
                    if (tmp_obj.GetComponent<SensorControl>().getCollisionObj().tag != touch_mal.tag)
                    if (row >= 4 && row <= 6) {
                        if (col >= 1 && col <= 3) // 초의 왕궁
                            if (touch_mal.tag.Equals("Han")) {
                                gameDirector.setAttackedSensorPosition(row, col);
                            }

                            else if (col >= 8 && col <= 10) { // 한의 왕궁
                                if (touch_mal.tag.Equals("Cho")) {
                                    gameDirector.setAttackedSensorPosition(row, col);
                                }
                            }
                    }
